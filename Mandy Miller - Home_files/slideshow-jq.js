...
function instantiateHeaderSlideshow(options) {
	// `wsite-header-section` are the "Header Sections" feature only
	var $headers = $('[class*="wsite-header"]:not(.wsite-header-section)');
	...
	if (slideshowElement) {
		var slideshow = new Slideshow(slideshowElement, {
			...
			images: options.images || options.slides,
...
function Slideshow(element, options) {
	...
	function destroy() {
		element.empty();
		// undo CSS set during render
		if (options.fillDimensions && options.useAspectRatio) {
			element.css('height', '');
		}
		element.removeClass('wslide');
...
function thumbnailURL(photo) {
	var url = photo.previewUrl || photo.url;
	if (!url) {
		return largeURL(photo);
	}
	if (!photo.previewUrl && !url.match('/weebly/images/') && !url.match('x-com-weebly-images')) {
		// old data - need to manipulate URL
...
function largeURL(photo) {
	var url = (window.inEditor && inEditor()) ?
		(photo.editorUrl || photo.url) :
		(photo.publishedUrl || photo.url);

	if (!photo.editorUrl && !url.match('/weebly/images/') && !url.match('x-com-weebly-images')) {
		// old data - need to manipulate URL
		url = '/uploads/' + url.replace(/^\/uploads\//, '');
